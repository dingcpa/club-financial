...
│   │   ├── composables/     # useFinance / useMembers / useDues
│   │   └── pages/           # 11 個頁面元件（.vue）
│   └── vite.config.js       # dev proxy /api → :5000
├── .env              # 本機 DB 連線設定（不進 git）
└── package.json      # build / start 腳本
```
...
四張資料表，由 `server/db.js` 的 `initDB()` 自動建立：
...
- **環境變數**：在 Zeabur Variables 頁面設定 DB_HOST / DB_PORT / DB_USER / DB_PASSWORD / DB_NAME
