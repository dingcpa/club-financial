    ...
    <v-card v-else elevation="1">
      <v-card-title class="d-flex flex-wrap justify-space-between align-center pa-4 ga-2">
        <div class="d-flex align-center ga-2">
          ...
          <span class="text-h6 font-weight-bold">社友名冊管理</span>
        </div>
      ...
      <v-card-text v-if="showForm">
        <v-card variant="outlined" color="primary" class="pa-4 mb-4" rounded="lg">
      ...
      <div style="overflow-x:auto">
        <v-table density="compact" style="min-width:850px;font-size:0.8rem">
          <thead>
            <tr>
              <th style="min-width:80px">姓名</th>
              <th style="width:80px">職稱</th>
              <th style="width:70px">社名</th>
              <th style="width:90px">生日</th>
              <th style="width:170px">聯絡資訊</th>
              <th style="max-width:140px">地址</th>
          ...
          <tbody>
            ...
            <tr v-for="(member, i) in filteredMembers" :key="member.id" :class="i % 2 === 0 ? 'bg-white' : 'bg-grey-lighten-5'">
              ...
              <td class="text-caption text-medium-emphasis">{{ member.birthday }}</td>
              <td>
                <div class="text-caption d-flex flex-column ga-1">
                  <div><v-icon size="10" color="primary">mdi-cellphone</v-icon> {{ member.mobile || '-' }}</div>
                  <div><v-icon size="10" color="grey">mdi-phone</v-icon> {{ member.phone || '-' }}</div>
                  <div><v-icon size="10" color="grey-lighten-1">mdi-email</v-icon> {{ member.email || '-' }}</div>
                </div>
              </td>
              <td class="text-caption text-medium-emphasis" style="max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" :title="member.address">
                {{ member.address || '-' }}
              </td>
              <td>
