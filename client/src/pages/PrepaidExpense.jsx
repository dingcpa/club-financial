...
import { Briefcase, ArrowRight, History, Calendar } from 'lucide-react';

const MONTHS = Array.from({ length: 12 }, (_, i) => i + 1);

function PrepaidExpense({ records, loading }) {
    ...
    records.filter(r => r.type === 'expense').forEach(record => {
        ...
        totalPrepaidAsset += remainingAmount;
    });
        ...
        <div>
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
                <h3 style={{ fontSize: '1.5rem', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <Briefcase size={24} color="var(--danger)" /> {toMinguoYear(selectedYear)}年 {selectedMonth}月 預付支出明細表
                </h3>
            ...
            <div className="dashboard-header" style={{ marginBottom: '2rem' }}>
                <div className="glass-card stat-card" style={{ flex: 1, background: 'linear-gradient(135deg, #ef4444 0%, #f97316 100%)', color: 'white' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        <Briefcase size={32} />
                        <div>
                            <div style={{ opacity: 0.8, fontSize: '0.875rem' }}>截至 {toMinguoYear(selectedYear)}年{selectedMonth}月 預付支出餘額</div>
                            <div style={{ fontSize: '1.75rem', fontWeight: '800' }}>NT$ {Math.round(totalPrepaidAsset).toLocaleString()}</div>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th style={{ textAlign: 'right' }}>已轉支出 (累計)</th>
                                <th style={{ textAlign: 'right', color: 'var(--danger)' }}>剩餘預付款項</th>
                            </tr>
                        </thead>
                        <tbody>
                            {prepaidRecords.length === 0 ? (
                                ...
                                prepaidRecords.sort((a, b) => {
                                    ...
                                    <tr key={r.id} style={{ opacity: r.remaining === 0 ? 0.6 : 1 }}>
                                        ...
                                        <td style={{ textAlign: 'right', color: 'var(--success)' }}>{Math.round(r.recognized).toLocaleString()}</td>
                                        <td style={{ textAlign: 'right', fontWeight: 'bold', color: r.remaining > 0 ? 'var(--danger)' : 'var(--text-muted)' }}>
            ...
            <div style={{ marginTop: '1.5rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div className="glass-card" style={{ padding: '1rem', display: 'flex', alignItems: 'start', gap: '0.75rem' }}>
                    ...
                    <div>
                        <div style={{ fontWeight: '700', fontSize: '0.9rem' }}>已轉支出 (累計)</div>
                ...
                <div className="glass-card" style={{ padding: '1rem', display: 'flex', alignItems: 'start', gap: '0.75rem' }}>
                    <ArrowRight size={20} color="var(--danger)" />
                    <div>
                        <div style={{ fontWeight: '700', fontSize: '0.9rem' }}>預付款項餘額</div>
                        <p style={{ margin: '0.25rem 0 0', fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                            截至 <strong>{toMinguoYear(selectedYear)}年 {selectedMonth}月</strong> 尚未轉入費用的資產餘額。
                        </p>
                    </div>
                </div>
            </div>
