    ...
    <div v-else>
      <div class="d-flex flex-wrap justify-space-between align-center mb-6 ga-3">
        <h3 class="text-h6 font-weight-bold d-flex align-center ga-2">
          ...
          {{ toMinguoYear(selectedYear) }}年 {{ selectedMonth }}月 預付支出明細表
        </h3>
      ...
      <v-card class="mb-6" elevation="0" rounded="lg" style="background:linear-gradient(135deg,#6366f1,#8b5cf6)">
        <v-card-text class="text-white d-flex align-center ga-4">
          <v-icon size="40">mdi-book-open-variant</v-icon>
          <div>
            <div style="opacity:.8" class="text-body-2">截至 {{ toMinguoYear(selectedYear) }}年{{ selectedMonth }}月 預付費用餘額</div>
            <div class="text-h5 font-weight-bold">NT$ {{ Math.round(totalPrepaid).toLocaleString() }}</div>
          </div>
        </v-card-text>
      </v-card>
          ...
          <v-card elevation="0" variant="tonal" color="primary" class="pa-3">
            <div class="d-flex ga-2">
              ...
              <div>
                ...
                <p class="text-caption text-medium-emphasis mb-0">截至 <strong>{{ toMinguoYear(selectedYear) }}年 {{ selectedMonth }}月</strong> 已轉入各月支出報表中的認列金額。</p>
              </div>
            </div>
          </v-card>
        </v-col>
        <v-col cols="12" md="6">
          <v-card elevation="0" variant="tonal" color="deep-purple" class="pa-3">
            <div class="d-flex ga-2">
              <v-icon>mdi-arrow-right</v-icon>
              <div>
                <div class="font-weight-bold text-body-2">預付費用餘額</div>
                <p class="text-caption text-medium-emphasis mb-0">截至 <strong>{{ toMinguoYear(selectedYear) }}年 {{ selectedMonth }}月</strong> 尚未認列為支出的預付項餘額。</p>
              </div>
            </div>
          </v-card>
        </v-col>
...
const totalPrepaid = computed(() => prepaidRows.value.reduce((s, r) => s + r.remaining, 0))
</script>
